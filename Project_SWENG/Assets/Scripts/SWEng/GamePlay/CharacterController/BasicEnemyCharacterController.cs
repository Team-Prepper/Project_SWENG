...
public class BasicEnemyCharacterController : MonoBehaviour, ICharacterController
{
    ...
    private void DoMove()
    {
        ...
        IPathGroup movementRange = EntityManager.Instance.
            GetPathGroupTo(_cc.EntityTransform.Pos,
                pos.Value, _cc.DicePoint.GetPoint() + 2);

        IList<GridCoord2D> pathHex = movementRange.GetPathTo(pos.Value);
        IList<GridCoord2D> path = pathHex.ToList();
