...
public struct HexCoordinate : IEquatable<HexCoordinate> {
    ...
    {
        if (x % 2 == 0)
            return new Vector3(x * HexGrid.Instance.XOffset, 0, z * HexGrid.Instance.ZOffset);
        return new Vector3(x * HexGrid.Instance.XOffset, 0, (z - 0.5f) * HexGrid.Instance.ZOffset);
    }
    ...
    {
        int x = Mathf.RoundToInt(source.x / HexGrid.Instance.XOffset);
        int z = Mathf.CeilToInt(source.z / HexGrid.Instance.ZOffset);
