...
public class PhotonCharacterController : MonoBehaviourPun, ICharacterController {
    ...
    public void Attack(IList<HexCoordinate> targetPos, int dmg, float time)
    {
        ...
        _view.RPC("_AttackAct", RpcTarget.All, time, targetPos.Count);
    }

    [PunRPC]
    private void _AttackAct(float time, int attackCount)
    {
    ...
    public void CamSetting()
    {
        ...
        _view.RPC("_CamSetting", RpcTarget.All);

        Debug.Log("CamSetting");

    }

    [PunRPC]
    private void _CamSetting()
    {
        CamMovement.Instance.SetCamTarget(transform);
        CamMovement.Instance.ConvertToCharacterCam();
