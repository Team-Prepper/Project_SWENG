...
public class CharacterMove : MonoBehaviour, ICharacterComponent {
    ...
    public void TryAddAction(IList<IActionSelector.Action> target) {
        if (_cc.DicePoint.GetPoint() < 2) return;

        target.Add(IActionSelector.Action.Move);
    }

    public void Move(Queue<Vector3> path, Action<HexCoordinate, HexCoordinate> moveAction)
    {
        StartCoroutine(_RotationCoroutine(path, _moveRotationDuration, moveAction));

    }

    private IEnumerator _RotationCoroutine(Queue<Vector3> path, float rotationDuration, Action<HexCoordinate, HexCoordinate> moveAction)
    {
        ...
        foreach (Vector3 targetPos in path)
        {
            ...
            moveAction?.Invoke(
                HexCoordinate.ConvertFromVector3(startPosition),
                HexCoordinate.ConvertFromVector3(targetPos));
        }
