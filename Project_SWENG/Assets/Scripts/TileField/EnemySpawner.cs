...
public class EnemySpawner : MonoBehaviour
{
    ...
    public List<GameObject> _bossEnemyPrefabList;
    ...
    public void SpawnEnemy()
    {
        ...
        {
            SpawnEnemy(GetRandHex(), _bossEnemyPrefabList);
        }
        for (int i = 0; i < enemyCnt; i++)
        {
            SpawnEnemy(GetRandHex(), _enemyPrefabList);
        }
    }

    private void SpawnEnemy(MapUnit spawnHex, List<GameObject> spawnEnemyList)
    {
        ...
        cc.Initial(spawnEnemyList[Random.Range(0, spawnEnemyList.Count)].name, 1, true);
